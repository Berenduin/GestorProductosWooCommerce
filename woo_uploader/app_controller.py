...
class AppController:
    ...
    def start(self) -> None:
        ...
        self.navigation = ft.NavigationRail(selected_index=0, label_type=ft.NavigationRailLabelType.ALL, bgcolor=PURPLE, indicator_color=GOLD, selected_label_text_style=ft.TextStyle(color=WHITE, weight=ft.FontWeight.BOLD), unselected_label_text_style=ft.TextStyle(color=WHITE), leading=ft.Container(content=ft.Image(src="el-baul-logo-white.png", width=92, height=70, fit=ft.ImageFit.CONTAIN), margin=ft.margin.only(bottom=12)), destinations=[ft.NavigationRailDestination(icon=ft.Icon(ft.Icons.UPLOAD_FILE, color=WHITE), selected_icon=ft.Icon(ft.Icons.UPLOAD_FILE, color=PURPLE), label="Subir"), ft.NavigationRailDestination(icon=ft.Icon(ft.Icons.SETTINGS, color=WHITE), selected_icon=ft.Icon(ft.Icons.SETTINGS, color=PURPLE), label="Configuración"), ft.NavigationRailDestination(icon=ft.Icon(ft.Icons.HELP_OUTLINE, color=WHITE), selected_icon=ft.Icon(ft.Icons.HELP, color=PURPLE), label="Ayuda")], on_change=lambda event: self.set_view(("upload", "settings", "help")[event.control.selected_index]))
    ...
    def set_view(self, view: str, upload_tab: int = 0, settings_tab: int = 0) -> None:
        ...
        self.navigation.selected_index = {"upload": 0, "settings": 1, "help": 2}[view]
        if view == "settings":
